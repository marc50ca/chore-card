...
def _get_coordinator(hass: HomeAssistant) -> ChoreTrackerCoordinator | None:
    entries = hass.data.get(DOMAIN, {})
    return next(iter(entries.values()), None)


# ── WebSocket API ──────────────────────────────────────────────────────────────
# The Lovelace card calls these directly — no entity ID guessing, no size limits.

def _register_websocket_api(hass: HomeAssistant) -> None:
    """Register WebSocket commands for the Lovelace card."""

    @websocket_api.websocket_command({vol.Required("type"): "chore_tracker/get_tasks"})
    @websocket_api.async_response
    async def ws_get_tasks(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict) -> None:
        coordinator = _get_coordinator(hass)
        if not coordinator or not coordinator.data:
            connection.send_result(msg["id"], {"tasks": {}, "categories": [], "stats": {}})
            return

        tasks = coordinator.data.get("tasks", {})
        cats  = coordinator.data.get("categories", [])
        stats = coordinator.data.get("stats", {})

        # Serialize any date objects → strings
        serialized = {}
    ...
    @websocket_api.websocket_command({vol.Required("type"): "chore_tracker/subscribe"})
    @websocket_api.async_response
    async def ws_subscribe(hass: HomeAssistant, connection: websocket_api.ActiveConnection, msg: dict) -> None:
        """Subscribe to task updates — card gets notified on every coordinator refresh."""
        coordinator = _get_coordinator(hass)
        if not coordinator:
            connection.send_error(msg["id"], "not_found", "Chore Tracker not configured")
            return
        ...
        def _forward_update() -> None:
            ...
            connection.send_event(msg["id"], {
                "tasks":      serialized,
                "categories": coordinator.data.get("categories", []),
                "stats":      coordinator.data.get("stats", {}),
            })
        ...
        # Register listener — fires whenever coordinator refreshes
        remove_listener = coordinator.async_add_listener(_forward_update)

        # Clean up when the WS connection closes
