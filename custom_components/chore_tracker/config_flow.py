...
class ChoreTrackerConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_m365_creds(self, user_input=None) -> FlowResult:
        ...
        if user_input is not None:
            ...
            # Build the redirect URI using the external URL.
            # Try external first, then internal, then the known fallback.
            ha_url = None
            for kwargs in [
            ...
            if not ha_url:
                ha_url = (
                    ...
                    or "https://homeassistant.peterborough.madasc.com:8123"
                )
